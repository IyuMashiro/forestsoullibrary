$(function () {
    ...
    $.ajax({
        ...
        success: function (json) {
            if (json.state === 200) {
                $("#bookName").val(json.data.bookName);
                $("#bookAuthor").val(json.data.bookAuthor);
                $("#bookPrice").val(json.data.bookPrice);
                $("#bookInventory").val(json.data.bookInventory);
                $("#inputDate").val(json.data.bookDate);
                $("#type").val(json.data.bookType);
            } else {
                alert("出现未知异常")
            }
        },
    ...
    $("#register-form-sub").click(function (){
        $.ajax({
            url:"/book/update",
            type:"POST",
            data:"id="+id+"&"+$("#form").serialize()+"&bookImg="+iconUrl+"&bookPdf="+pdfUrl,
            dataType:"JSON",
            success:function (json){
                if (json.state === 200){
